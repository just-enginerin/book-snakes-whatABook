...
def displayCustomerWishlist(customerId):
    ...
    if specified_customer['wishlistItems'] == []:
        ...
        print(specified_customer['firstName'] + ' ' +
              specified_customer['lastName'] + "'s wishlist is empty.")
...
def removeBookFromCustomerWishlist(customerId, bookId):
    ...
    if specified_customer['wishlistItems'] == []:
        ...
        print('Cannot remove ' + specified_book['title'] + '. ' + specified_customer['firstName'] +
              ' ' + specified_customer['lastName'] + "'s wishlist is empty!")
        ...
        for item in wishlist_items:
            if item.get('bookId') == bookId:
                ...
                db.customers.update_one({'_id': specified_customer['_id']}, {
                                        '$set': {'wishlistItems': wishlist_items}})
                print(specified_book['title'] + ' has been successfully removed from ' +
                      specified_customer['firstName'] + ' ' + specified_customer['lastName'] + "'s wishlist!")
                break
        else:
            # Error message: the book cannot be removed because it is not on the customer's wishlist.
            print(specified_book['title'] + ' is not on ' + specified_customer['firstName'] +
                  ' ' + specified_customer['lastName'] + "'s wishlist!")
